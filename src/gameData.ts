...
export const BESTIARY: Monster[] = [
  ...
  },
  {
    ...
    stat: 2, wounds: 3, speed: "2/4 (고블린/사냥개 형상)",
...
export const WILDERNESS_EVENTS_KO = [
  ...
  "돈을 요구하는 도적 떼", "귀신 들린 묘지", "바실리스크 동굴에 갇힌 인물", "도적 떼의 기습",
  "임무의 이정표 또는 단서",
];

export const DUNGEON_EVENTS_KO = [
  "주변에서 색채가 빠져나간다", "압도적인 어둠", "먼 곳에 울리는 여분의 발소리", "거세게 흐르는 물소리",
  "썩은 시체로 가득한 방", "일행을 지켜보는 고블린 척후병", "길을 가로막는 넓은 균열", "지상으로 이어지는 좁은 수직 통로",
  "최근까지 타오른 화덕", "구출을 기다리는 우리 속 드워프", "함정을 알리는 찰칵 소리", "물약을 달이는 은둔자",
  "요정들이 취해 있는 아편굴", "우연히 들어선 지하 묘지", "바게스트와 고블린의 습격", "희생 의식을 치르는 사교도",
  "뼈를 가지고 노는 오우거", "추격받는 부상한 도둑", "던전 통로가 자리를 바꾼다", "저편 세계로 통하는 문",
  "드래곤이 다가온다",
];

export const SETTLEMENT_EVENTS_KO = [
  "떠돌이 비술사가 도착한다", "여러 아이가 한꺼번에 태어난다", "일행에 관한 소문이 퍼진다", "근처 거인에 관한 풍문",
  "성스러운 의식이 치러진다", "일주일 동안 이어지는 혼인 잔치", "전령이 좋은 소식을 가져온다", "핏빛 독사가 마을을 덮친다",
  "현자가 징조를 전한다", "촌장이 독에 중독된다", "격한 말다툼이 커진다", "죄를 물어 통 제작자를 교수형에 처한다",
  "낡은 종이 저절로 울린다", "모두가 같은 꿈을 꾼다", "여관 주인의 가보가 도난당한다", "서약 파기자들이 마을을 점거한다",
  "별이 마을 밖에 떨어진다", "아이들이 몽유하며 늪으로 향한다", "해가 지지 않는다", "종이 사람들의 이름을 울린다",
  "요정들이 선물을 들고 찾아온다",
];

export const MAP_WILDERNESS_KO = [
  "농경지", "방목지", "키 큰 풀밭", "연못", "요정의 샘", "호수", "고분 벌판", "완만한 구릉",
  "포도밭", "이탄 늪", "숲이 우거진 습지", "강의 범람원", "버섯 숲", "참나무 숲", "요정 들린 숲",
  "벌목된 숲", "헤더 황야", "고대 전장", "정착지—마을", "정착지—성 또는 요새", "정착지—도시",
];

export const MAP_DUNGEON_KO = [
  "빈 방", "레드캡 소굴", "부엌", "감옥 구역", "회합 장소", "위로 향하는 계단", "아래로 향하는 계단",
  "천연 동굴", "철광", "이글거리는 대장간", "버섯 농장", "지하 강", "고블린 군락", "드워프 정착지",
  "괴수의 소굴", "지하 호수", "병영", "납골당", "요정 선술집", "고문실", "제단",
];
...
export const FOLK_ROAD_KO = {
  occupations: [
    ...
    "폐허 탐사자", "점성술사", "라이칸스로프", "농부", "촌장", "전승 학자",
  ],
  personalities: [
...
export const MAGICK_ITEM_TEXT_KO: Record<keyof typeof MAGICK_ITEMS, string[]> = {
  Swords: [
    ...
    "거인과 오우거·트롤·위커맨 같은 비슷한 괴수를 상대로 부상 3점을 입힌다.",
